...
- [x] 1. Corrigir bugs existentes
  - [x] 1.1 Corrigir `validate_number` em `libs/utils/validate.sh`
    - Substituir `$this_id` e `$this_table` por `$this_number` nas mensagens de log da função `validate_number`
    - _Requirements: 9.1_

  - [x] 1.2 Corrigir `system_install` em `libs/utils/system.sh`
  ...
  - [x] 1.3 Corrigir roteamento `organize` em `plan.sh`
    - Alterar `clarify_main` para `organize_main "$@"` no case `organize`
    - _Requirements: 8.3, 9.3_

- [x] 2. Atualizar schema SQL em `libs/database/database_init.sql`
  - [x] 2.1 Adicionar tabela `areas` e view `areas_view`
  ...
  - [x] 2.2 Adicionar tabela `projects` e view `projects_view`
  ...
  - [x] 2.3 Atualizar tabela `tasks` e view `tasks_view`
...
- [x] 3. Implementar `libs/objects/area.sh`
  - [x] 3.1 Reescrever `area.sh` com funções snake_case
    - Implementar `area_add()`: arg posicional NAME obrigatório, flag `--description` opcional, chamar `generic_add "areas"`
    - Implementar `area_delete()`: verificar que NAME existe (`SELECT id FROM areas WHERE name = NAME`), `log_print user` para confirmação, executar `DELETE FROM areas WHERE name = NAME`
    - Implementar `area_list()`: chamar `generic_list areas_view`
    - Implementar `area_edit()`: verificar que NAME existe, parsear flags `--name` e `--description`, chamar `generic_set_property areas name NAME {field} '{value}'` para cada flag fornecida
    ...
    - Nota: area usa NAME como identificador na interface — nenhuma função aceita ID como argumento
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 4.8, 4.9_

  - [x] 3.2 Escrever teste de propriedade para `area_add` (Property 1)
    - **Property 1: Area add confirmation**
    - **Validates: Requirements 4.1**

  - [x] 3.3 Escrever teste de propriedade para `area_list` (Property 2)
    - **Property 2: Area list shows inserted areas**
    - **Validates: Requirements 4.4**

  - [x] 3.4 Escrever teste de propriedade para `area_edit` (Property 3)
    - **Property 3: Area edit updates field**
    - **Validates: Requirements 4.7, 4.8**

  - [x] 3.5 Escrever teste de propriedade para `area_delete` (Property 4)
    - **Property 4: Area delete removes record**
    - **Validates: Requirements 4.5**

- [x] 4. Checkpoint — Garantir que area funciona
...
- [x] 5. Implementar `libs/objects/project.sh`
  - [x] 5.1 Reescrever `project.sh` com funções snake_case
    - Implementar `project_add()`: arg posicional NAME obrigatório, `--area AREA_NAME` **obrigatório** (erro se ausente), flags opcionais `--start-date`, `--due-date`; resolver AREA_NAME → area_id via `SELECT id FROM areas WHERE name = AREA_NAME` (erro se não encontrado); validar datas via `validate_date`; chamar `generic_add "projects"` com INSERT único (sem `--goal` por ora)
  ...
  - [ ]* 5.3 Escrever teste de propriedade para validação de area por nome em project (Property 6)
...
- [x] 6. Checkpoint — Garantir que project funciona
...
- [x] 7. Completar `libs/objects/task.sh`
  - [x] 7.1 Reescrever `task_add()` com INSERT único e suporte a `--project`
  ...
  - [x] 7.2 Implementar funções ausentes em `task.sh`
  ...
  - [x] 7.3 Remover funções camelCase legadas de `task.sh`
...
- [x] 8. Checkpoint — Garantir que task funciona
...
- [x] 9. Adicionar help messages em `libs/utils/help.sh`
  - [x] 9.1 Implementar `help_print_area()` e `help_print_area_add()`
  ...
  - [x] 9.2 Implementar `help_print_project()` e `help_print_project_add()`
    - `help_print_project`: exibir nome, descrição, uso e subcomandos (`add`, `complete`, `delete`, `edit`, `list`, `search`, `start`, `stop`)
    - `help_print_project_add`: exibir uso, argumento obrigatório `NAME`, flag obrigatória `--area AREA_NAME` e flags opcionais `--start-date`, `--due-date`
    - _Requirements: 7.3, 7.4, 7.5_

- [x] 10. Adicionar roteamento de `area` e `project` em `plan.sh`
...
- [x] 11. Adicionar testes unitários em `tests/unit-tests.sh`
  - [x] 11.1 Adicionar testes de exemplo para Area
  ...
  - [x] 11.2 Adicionar testes de exemplo para Project
  ...
  - [x] 11.3 Adicionar testes de exemplo para Task com project
  ...
  - [x] 11.5 Adicionar teste de roteamento do organize
    - `test_command_output "Organize routes correctly" "./plan.sh organize" "Lowbit Planner"`
    - _Requirements: 8.3_

- [x] 12. Atualizar help principal em `libs/utils/help.sh`
  - Adicionar `area` e `project` na seção OBJECT COMMANDS de `help_print_main`
  - Exemplo: `printf "  area           ${color_gray}Manage areas of responsibility (Horizon 2)${color_reset}\n"`
  - Exemplo: `printf "  project        ${color_gray}Manage projects (Horizon 1)${color_reset}\n"`
